<?php
	...
	if (array_key_exists('get-dims', $_GET)) {
		ob_end_clean();
		$tmp = $lvObject->domainGetScreenDimensions($name);
		if (!$tmp)
			die( $lvObject->getLastError() );
	...
	if (array_key_exists('x', $_GET)) {
		ob_end_clean();
		$tmp = $lvObject->domainSendPointerEvent($name, $_GET['x'], $_GET['y'], (int)$_GET['bm'], $_GET['autorelease'] ? true : false);
		if (!$tmp)
			die( $lvObject->getLastError() );
	...
	if (array_key_exists('send_keys', $_GET)) {
		ob_end_clean();
		$tmp = $lvObject->domainSendKeys($name, $_GET['send_keys']);
		if (!$tmp)
			die( $lvObject->getLastError() );
	...
	$msg = '';
	if (!$lvObject->domainIsRunning($name))
		$msg = $lang->get('domain-shutoff');
	...
	if (array_key_exists('data', $_GET) && ($_GET['data'] == 'png')) {
		ob_end_clean();
		if ($lvObject->domainIsRunning($name)) {
			$tmp = $lvObject->domainGetScreenshot($name);
			if (is_array($tmp)) {
				$data = $tmp['data'];
				$mime = $tmp['mime'];
			...
			die( 'Error: Domain is not running' );
	}
?>
  <!-- CONTENTS -->
  <div id="content">
...
<?php
	if ($msg):
?>
    <div class="section"><?php echo $lang->get('domain-screenshot') ?></div>
    <div id="msg-error"><b><?php echo $lang->get('msg') ?>: </b><?php echo $msg ?></div>
<?php
	else:
		$dims = $lvObject->domainGetScreenDimensions($name);
?>
...
?>
    ...
    <div class="item">
      <div class="label"><?php echo $lang->get('interval-sec') ?>:</div>
      <div class="value">
